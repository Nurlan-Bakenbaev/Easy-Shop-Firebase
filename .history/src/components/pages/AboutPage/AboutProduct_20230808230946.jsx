...
const AboutProduct = () => {
  ...
  useEffect(() => {
    const getAboutData = async () => {
      const aboutItem = doc(db, "headPhones","phones", id);
      const data = await getDoc(aboutItem);
      const headPhonesData = await getDoc(headPhonesRef);


      if (data.exists()) {
        const phonesDocumentId = headPhonesData.data().phonesDocumentId;
        const phonesRef = doc(db, "phones", phonesDocumentId);
        const phonesData = await getDoc(phonesRef);
  
        if (phonesData.exists()) {
          const el = {
            id: headPhonesData.id,
            ...headPhonesData.data(),
            phonesData: phonesData.data(),
          };
          setAboutData(el);
        }
      }
