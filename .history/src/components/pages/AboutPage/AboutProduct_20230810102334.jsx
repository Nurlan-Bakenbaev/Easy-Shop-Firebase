...
const AboutProduct = () => {
  ...
  useEffect(() => {
    const getAboutData = async () => {
      ...
      const wirelessHeadPhonesRef = doc(db, "phones", id);


      const data = await getDoc(aboutItem);
      const aboutIphonesData = await getDoc(aboutIphonesRef);
      const wirelessHeadPhones = await getDoc(wirelessHeadPhonesRef);
    ...
    }else if (wirelessHeadPhones.exists()){
      setAboutData({
        id:wirelessHeadPhones.id,
        ...wirelessHeadPhones.data()
      })
    }
  };
    getAboutData();
  ...
  return (
    <>
      ...
      <div className="about__card">
        <AboutCard
          aboutImg={aboutData.images ? aboutData.images.map((image) => (
