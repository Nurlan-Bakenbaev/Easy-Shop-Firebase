...
const Home = ({ handleClick, handleFavorite }) => {
  ...
  useEffect(()=>{
    function getWirelessPhonesDataFromFireBase(){
    try{
      const wirelessData = getDocs(wirelessHeadPhoneDataRef)
      const items = wirelessData.docs.map((doc)=>({
        ...doc.data(),
        id:doc.id
      }))
