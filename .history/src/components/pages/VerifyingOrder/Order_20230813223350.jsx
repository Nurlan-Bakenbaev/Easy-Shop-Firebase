...
const Order = ({ cartItem }) => {
  ...
  const OrderForm = (
    <div className="order-wrapper"><h1>Оформление заказа</h1>
      <form className="order" onClick={()=>handleSubmitForm}>
          ...
          <div className="order-form">
            ...
            <div className="form">
              <div className="address">
                <input type="text"placeholder="Город"value={formData.city}name="city" onChange={handleInputChange} />
                <div className="region__street">
                  <input type="text" placeholder="Улица/Район" value={formData.street}name="street" onChange={handleInputChange}/>
                  <EditIcon />
                </div>
              </div>
              ...
              <div className="house__number">
                <div>
                  <input type="number" placeholder="Дом"value={formData.house}name="house" onChange={handleInputChange} />
                  <EditIcon />
                </div>
                <div>
                  <input type="number" placeholder="Квартира"value={formData.flat}name="flat" onChange={handleInputChange} />
                  <EditIcon />
                </div>
              </div>
            </div>
          </div>
        </div> <div className="cost__items">
        <h3>Ваш Заказ</h3>
        {cartItem.map((el) => {
          return (
            <ul key={el.id}>
        ...
        <div>
          ...
          <div className="client__contact">
            <input type="text" placeholder="0996 "value={formData.phoneNumber}name="phoneNumber" onChange={handleInputChange} />
          </div>
        </div>
